...
const createAxiosInstance1 = (token: string | null): AxiosInstance => {
    if (token) {
        return axios.create({
            baseURL: API_URL1,
            headers: {
...
export const getRequest1 = async (url: string): Promise<any> => {
    try {
        const token = localStorage.getItem("jwtToken")
        const axiosInstance: AxiosInstance = createAxiosInstance1(token)
