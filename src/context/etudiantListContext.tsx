...
import { deleteRequest, getRequest, postRequest, putRequest} from "../api/axios";
...
const addNewEtudiant = useCallback(
    async (etudiantBody: EtudiantDTO,anneeUniversitaire: number, codeFormation: string) => {
...
const getEtudiant = useCallback(async (no_etudiant:string) => {
    try {
        const response: ApiResponse = await getRequest(`/etudiant/${no_etudiant}`);
        const { data } = response;
        return data; 
    } catch (error) {
        console.error("Une erreur s'est produite lors de la récupération de l'étudiant");
        throw error; 
    }
}, []);
...
const modifyEtudiant = useCallback(
    async (no_etudiant: number, etudiantBody: EtudiantDTO,anneeUniversitaire: number, codeFormation: string) => {
...
const removeEtudiant = useCallback(
    async (no_etudiant: number,anneeUniversitaire: number, codeFormation: string) => {
