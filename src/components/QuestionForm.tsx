...
import { Qualificatif } from "../types"
...
const QuestionForm: React.FC<questionFormProps> = ({ add }) => {
    ...
    const { qualificatifList } = useContext(QualificatifContext)

    const [selectedQualificatif, setSelectedQualificatif] =
        React.useState<string>(coupleQualificatif)
    useEffect(() => {
        setSelectedQualificatif(coupleQualificatif)
    }, [coupleQualificatif])
    //  const [initialIntitule, setInitialIntitule] = useState<any | undefined>(undefined);

    const transformedQuestionListe = qualificatifList.map(
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (add === true) {
            ...
            if (!selectedQualificatif) {
                showNotification(
                    "Erreur",
                    "Veuillez choisir un qualificatif",
                    "error"
                )
                return
            }
        ...
        >
            ...
            <Box sx={{ display: "flex", gap: "1rem" }}>
                <Select
                    ...
                    onChange={(value) =>
                        setSelectedQualificatif(value as string)
                    }
                    required
                    multiple={false}
                    //  sx={{ width: "50%" }} // Ajustez la largeur comme vous le souhaitez
